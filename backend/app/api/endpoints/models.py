...
class ModelRegisterRequest(BaseModel):
    version_name: str = Field(..., description="Unique filename/version of the weights file")
    file_path: Optional[str] = Field(None, description="Absolute file path on server disk")
    deployment_status: str = Field("staging", description="Initial status: staging | training | validated")
    ...
    dataset_name: Optional[str] = Field("General-Nika", description="Dataset used for training")
    trained_by: Optional[str] = Field("Operator", description="User who started training job")
    framework: Optional[str] = Field("Ultralytics YOLOv8", description="ML library used")
    commit_hash: Optional[str] = Field(None, description="Git commit hash of configuration repo")
    model_size_mb: Optional[float] = Field(None, description="Model file size in megabytes")
    parameter_count: Optional[int] = Field(None, description="Model parameter count")
    parent_version: Optional[str] = Field(None, description="Model checkpoint predecessor version")
...
class ModelStatusUpdateRequest(BaseModel):
    ...
    status: str = Field(..., description="New lifecycle state: training | validated | staging | production | archived")


# ── Routes ────────────────────────────────────────────────────────────────────

@router.get("", response_model=ModelListResponse, summary="List versioned model weight files and metadata")
...
@router.post("/register", response_model=ModelVersionSchema, status_code=status.HTTP_201_CREATED, summary="Register new model checkpoint")
def register_model_checkpoint(req: ModelRegisterRequest, db: Session = Depends(get_db)):
    ...
    new_ver = ModelVersion(
        ...
        artifact_path=req.file_path or str(model_registry.registry_dir / req.version_name),
...
def switch_model_version(version_name: str, db: Session = Depends(get_db)):
    ...
    return {"success": True, "detail": f"Active model successfully promoted and switched to {version_name}."}

@router.post("/status", response_model=ModelVersionSchema, summary="Update model deployment lifecycle state")
def update_model_status(req: ModelStatusUpdateRequest, db: Session = Depends(get_db)):
    ...
    if req.status == "production":
        ...
        try:
            updated = model_version_repo.set_deployment_status(db, req.version_name, req.status)
