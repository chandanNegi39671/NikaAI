...
from app.services.chat_assistant import ask_factory_assistant, get_session_history, clear_session_history
...
class AssistantQuery(BaseModel):
    question: str
    session_key: Optional[str] = Field(None, description="Persisted chat session key for loading history context")
...
class KnowledgeDocCreateRequest(BaseModel):
    ...
    doc_type: str = Field("manual", description="doc_type: manual | sop | faq | maintenance")
...
@router.post("/ask", response_model=AssistantResponse, summary="Ask Factory AI Assistant")
...
@router.get("/history/{session_key}", response_model=List[MessageHistoryItem], summary="Get chat session history list")
...
@router.post("/knowledge", response_model=KnowledgeDocResponse, status_code=status.HTTP_201_CREATED, summary="Create a new knowledge base manual", dependencies=[Depends(PermissionChecker("admin"))])
def create_knowledge_document(req: KnowledgeDocCreateRequest, db: Session = Depends(get_db)):
...
@router.get("/knowledge", response_model=List[KnowledgeDocResponse], summary="List active knowledge base documents")
def list_knowledge_documents(doc_type: Optional[str] = None, db: Session = Depends(get_db)):
    """Fetch active documents filterable by doc_type."""
    return knowledge_document_repo.list_active(db, doc_type=doc_type)

@router.get("/knowledge/search", response_model=List[KnowledgeDocResponse], summary="Keyword search knowledge documents")
def search_knowledge_documents(query: str, doc_type: Optional[str] = None, db: Session = Depends(get_db)):
