...
def get_inspection(id: str, db: Session = Depends(get_db)):
    ...
    ins = (
        db.query(Inspection)
        .filter(Inspection.id == id, Inspection.is_deleted == False)
        .first()
    )
    if not ins:
        raise HTTPException(
    ...
    return {
        ...
        "explanation": (
            {
                "gemma_explanation": (
                    ins.explanation.gemma_explanation if ins.explanation else "N/A"
                ),
                "trust_score": ins.explanation.trust_score if ins.explanation else 1.0,
                "explanation_json": (
                    ins.explanation.explanation_json if ins.explanation else None
                ),
            }
            if ins.explanation
            else None
