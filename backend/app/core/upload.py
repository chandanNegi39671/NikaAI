...
def allowed_file(filename: str, content_type: str) -> bool:
    ...
    if len(parts) > 2:
        logger.warning(f"File upload rejected: Double extension detected in '{filename}'")
        return False
    ...
    if ext not in valid_exts_for_mime:
        logger.warning(f"File upload rejected: Extension '{ext}' does not match MIME type '{content_type}'")
        return False
...
async def save_upload(upload_file: UploadFile, static_dir: Path) -> str:
    ...
    if not allowed_file(orig_filename, content_type):
        raise InvalidImageError("File type or extension validation failed. Upload rejected.")
