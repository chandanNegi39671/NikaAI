...
def get_dashboard_analytics(db: Session) -> dict:
    ...
    breakdown_query = db.query(
        Detection.defect_class, func.count(Detection.id)
    ).group_by(Detection.defect_class).all()
    ...
    for m in machines:
        ...
        m_defects = db.query(Inspection).filter(Inspection.machine_id == m.id, Inspection.status == "FAIL").count()
        m_defect_rate = round((m_defects / m_total * 100), 2) if m_total > 0 else 0.0
        machine_risks.append({
            "id": m.id,
            "name": m.name,
            "defectRate": m_defect_rate,
            "status": "Warning" if m_defect_rate > 10 else "Normal" if m_total > 0 else "Offline",
    ...
    for i in range(6, -1, -1):
        ...
        day_total = db.query(Inspection).filter(
            Inspection.created_at >= start_dt,
            Inspection.created_at <= end_dt
        ).count()
        
        day_defects = db.query(Inspection).filter(
            Inspection.created_at >= start_dt,
            Inspection.created_at <= end_dt,
            Inspection.status == "FAIL"
        ).count()
        
        day_avg_conf = db.query(func.avg(Inspection.confidence)).filter(
            Inspection.created_at >= start_dt,
            Inspection.created_at <= end_dt
        ).scalar() or 0.0
        
        day_avg_lat = db.query(func.avg(Inspection.latency_ms)).filter(
            Inspection.created_at >= start_dt,
            Inspection.created_at <= end_dt
        ).scalar() or 0.0
    ...
    for ins in recent_inspections:
        recent_activity.append({
            ...
            "status": ins.status,
            "defectName": ins.detections[0].defect_class.replace("_", " ").title() if ins.detections else "No Defects",
