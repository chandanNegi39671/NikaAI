...
from app.models.db_models import Detection, FactoryMemory, Inspection, Machine
...
def get_factory_memories(db: Session, query: str | None = None) -> list[dict]:
    ...
    for fm in results:
        ...
        total_occurrences = (
            db.query(Detection)
            .filter(
                Detection.defect_class == fm.defect_class, Detection.is_deleted == False
            )
            .count()
        )
        ...
        risk_machine = "None"
        top_machine = (
            db.query(Inspection.machine_id, func.count(Inspection.id).label("count"))
            .join(Detection, Detection.inspection_id == Inspection.id)
            .filter(
