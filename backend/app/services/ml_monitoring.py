...
def analyze_ml_performance(db: Session, limit: int = 100) -> dict:
    ...
    try:
        ...
        recent_inspections = (
            db.query(Inspection)
            .filter(Inspection.is_deleted == False)
        ...
        if confidence_drift > 0.10:
            status = "degraded"
            warnings.append(
                "Confidence drift detected: Model predictions show decreased accuracy."
            )
        ...
        if abs(reject_rate_drift) > 0.15:
            status = "degraded"
            warnings.append(
                "Reject rate drift detected: Production inputs might have changed."
            )
