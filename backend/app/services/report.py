...
def generate_inspection_pdf(db: Session, inspection_id: str) -> bytes:
    ...
    meta_data = [
        [
            Paragraph("Inspection ID:", bold_body_style),
            Paragraph(str(inspection.id), body_style),
        ],
        [
            Paragraph("Timestamp:", bold_body_style),
            Paragraph(
                inspection.created_at.strftime("%Y-%m-%d %H:%M:%S UTC"), body_style
            ),
        ],
        [
        ...
        [
            Paragraph("Operator:", bold_body_style),
            Paragraph(
                inspection.worker.name if inspection.worker else "Default", body_style
            ),
        ],
        [
            Paragraph("Machine Name:", bold_body_style),
            Paragraph(
                inspection.machine.name if inspection.machine else "Default", body_style
            ),
        ],
        [
            Paragraph("Inference Duration:", bold_body_style),
            Paragraph(f"{inspection.inference_time_ms:.1f} ms", body_style),
        ],
    ]
    ...
    if inspection.detections:
        ...
        story.append(
            Paragraph(
                "No defects detected. Part meets quality compliance criteria.",
                body_style,
            )
        )
    ...
    if inspection.explanation:
        ...
        story.append(
            Paragraph(f"<b>Trust Score:</b> {expl.trust_score * 100:.1f}%", body_style)
        )
        story.append(Spacer(1, 4))
