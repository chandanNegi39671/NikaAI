...
from app.core.provider_factory import get_knowledge_provider, get_llm_adapter
from app.core.repository import conversation_message_repo, conversation_repo

logger = get_logger(__name__)


def ask_factory_assistant(
    db: Session, question: str, session_key: str | None = None
) -> dict:
    ...
    if session_key:
        ...
        db_messages = conversation_message_repo.get_messages_for_conversation(
            db, conv.id, limit=10
        )
    ...
    if conversation_id:
        ...
        conversation_message_repo.add_message(
            db, conversation_id, "assistant", llm_response.answer
        )
