...
class TestModelRegistryAPI:
    ...
    def test_list_models(self, client: TestClient, admin_headers: dict, db_session: Session):
        # Seed models
    ...
    def test_register_model(self, client: TestClient, admin_headers: dict, db_session: Session):
        payload = {
        ...
        resp = client.post("/api/v1/models/register", json=payload, headers=admin_headers)
        ...
        db_item = db_session.query(ModelVersion).filter(ModelVersion.version_name == "new_registered_model.pt").first()
        assert db_item is not None
        assert db_item.deployment_status == "staging"

    def test_switch_model(self, client: TestClient, admin_headers: dict, db_session: Session):
        ...
        # Switch weights
        resp = client.post("/api/v1/models/switch?version_name=best.pt", headers=admin_headers)
        assert resp.status_code == 200
        data = resp.json()
        ...
        m_db = db_session.query(ModelVersion).filter(ModelVersion.version_name == "best.pt").first()
        assert m_db.deployment_status == "production"

    def test_switch_model_file_not_found(self, client: TestClient, admin_headers: dict, db_session: Session):
        ...
        resp = client.post("/api/v1/models/switch?version_name=nonexistent.pt", headers=admin_headers)
        assert resp.status_code == 400
        assert "Failed to switch" in resp.json()["detail"]

    def test_update_lifecycle_status(self, client: TestClient, admin_headers: dict, db_session: Session):
...
class TestAICopilotAPI:
    ...
    def test_ask_assistant_rule_based_response(self, client: TestClient, operator_headers: dict, db_session: Session):
        payload = {
            "question": "What is the SOP for surface cracks?",
            "session_key": "test_sess_1"
        }
        resp = client.post("/api/v1/assistant/ask", json=payload, headers=operator_headers)
        assert resp.status_code == 200
        data = resp.json()
        ...
        conv = db_session.query(Conversation).filter(Conversation.session_key == "test_sess_1").first()
        assert conv is not None
        messages = db_session.query(ConversationMessage).filter(ConversationMessage.conversation_id == conv.id).all()
        assert len(messages) == 2
        assert messages[0].role == "user"
        assert messages[1].role == "assistant"

    def test_get_and_clear_assistant_history(self, client: TestClient, operator_headers: dict, db_session: Session):
        # Create history directly
        conv = Conversation(session_key="test_sess_2")
        db_session.add(conv)
        db_session.flush()

        m1 = ConversationMessage(conversation_id=conv.id, role="user", content="Test prompt")
        m2 = ConversationMessage(conversation_id=conv.id, role="assistant", content="Test answer")
        db_session.add(m1)
        db_session.add(m2)
        db_session.flush()

        # Fetch history
        resp_get = client.get("/api/v1/assistant/history/test_sess_2", headers=operator_headers)
        ...
        # Clear history
        resp_del = client.delete("/api/v1/assistant/history/test_sess_2", headers=operator_headers)
    ...
    def test_knowledge_base_management_admin_only(self, client: TestClient, admin_headers: dict, operator_headers: dict, db_session: Session):
        payload = {
        ...
        resp_block = client.post("/api/v1/assistant/knowledge", json=payload, headers=operator_headers)
        assert resp_block.status_code == 403

        # 2. Allow admin to create documents
        resp_create = client.post("/api/v1/assistant/knowledge", json=payload, headers=admin_headers)
        ...
        resp_search = client.get("/api/v1/assistant/knowledge/search?query=buffer", headers=operator_headers)
...
class TestInferenceHistoryAPI:
    ...
    def test_query_inference_history_filters(self, client: TestClient, operator_headers: dict, db_session: Session):
        ...
        # Filter by status
        resp_status = client.get("/api/v1/inference/history?status=FAIL", headers=operator_headers)
        assert resp_status.status_code == 200
        assert resp_status.json()["total"] == 1
        assert resp_status.json()["results"][0]["id"] == ins2.id

        # Filter by defect category
        resp_defect = client.get("/api/v1/inference/history?defect_class=crack", headers=operator_headers)
        assert resp_defect.status_code == 200
        assert resp_defect.json()["total"] == 1

        # Single inspection details retrieve
        resp_detail = client.get(f"/api/v1/inference/history/{ins2.id}", headers=operator_headers)
...
class TestComplianceAuditLogsAPI:
    ...
    def test_query_audit_logs_rbac_filters(self, client: TestClient, admin_headers: dict, operator_headers: dict, db_session: Session):
        # Seed audit entries
        log1 = AuditLog(
        ...
        # 3. Filter query
        resp_filter = client.get("/api/v1/audit?request_id=req_1", headers=admin_headers)
...
class TestVisualizationEngineAPI:
    ...
    def test_get_visualization_report(self, client: TestClient, operator_headers: dict, db_session: Session):
        ...
        resp = client.get(f"/api/v1/visualization/report/{ins.id}", headers=operator_headers)
        assert resp.status_code == 200
        data = resp.json()
